...
export function TherapistOnboardingChecklist() {
  const [isVisible, setIsVisible] = useState(true)
  const [checklist, setChecklist] = useState<ChecklistItem[]>([
    {
      id: "profile",
      title: "Complete seu perfil",
      description: "Adicione suas informações profissionais e foto",
      completed: false,
    ...
    },
    {
      ...
      description: "Cadastre um cliente para começar a agendar sessões",
      completed: false,
    ...
    },
    {
      ...
      description: "Crie sua primeira sessão com um cliente",
      completed: false,
    ...
    },
    {
      ...
      description: "Defina seus métodos de pagamento e valores",
      completed: false,
  ...
  useEffect(() => {
    const storedChecklist = localStorage.getItem("therapist-onboarding-checklist")
    if (storedChecklist) {
      setChecklist(JSON.parse(storedChecklist))
    }

    const checklistHidden = localStorage.getItem("therapist-onboarding-checklist-hidden")
  ...
  const toggleItem = (id: string) => {
    ...
    localStorage.setItem("therapist-onboarding-checklist", JSON.stringify(updatedChecklist))
  }

  const hideChecklist = () => {
    setIsVisible(false)
    localStorage.setItem("therapist-onboarding-checklist-hidden", "true")
  }

  const resetChecklist = () => {
    const resetItems = checklist.map((item) => ({ ...item, completed: false }))
    setChecklist(resetItems)
    localStorage.setItem("therapist-onboarding-checklist", JSON.stringify(resetItems))
    setIsVisible(true)
    localStorage.removeItem("therapist-onboarding-checklist-hidden")
  }
  ...
  return (
    <Card className="mb-6">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Complete estas tarefas para configurar sua conta</CardDescription>
